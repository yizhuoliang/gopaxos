...
// NodeSimulatorClient is the client API for NodeSimulator service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
type NodeSimulatorClient interface {
	Sent(ctx context.Context, in *Message, opts ...grpc.CallOption) (*Message, error)
	Received(ctx context.Context, in *Message, opts ...grpc.CallOption) (*Message, error)
}

type nodeSimulatorClient struct {
	cc grpc.ClientConnInterface
}

func NewNodeSimulatorClient(cc grpc.ClientConnInterface) NodeSimulatorClient {
	return &nodeSimulatorClient{cc}
}

func (c *nodeSimulatorClient) Sent(ctx context.Context, in *Message, opts ...grpc.CallOption) (*Message, error) {
	out := new(Message)
	err := c.cc.Invoke(ctx, "/gopaxos.NodeSimulator/Sent", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *nodeSimulatorClient) Received(ctx context.Context, in *Message, opts ...grpc.CallOption) (*Message, error) {
	out := new(Message)
	err := c.cc.Invoke(ctx, "/gopaxos.NodeSimulator/Received", in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// NodeSimulatorServer is the server API for NodeSimulator service.
// All implementations must embed UnimplementedNodeSimulatorServer
// for forward compatibility
type NodeSimulatorServer interface {
	Sent(context.Context, *Message) (*Message, error)
	Received(context.Context, *Message) (*Message, error)
	mustEmbedUnimplementedNodeSimulatorServer()
}

// UnimplementedNodeSimulatorServer must be embedded to have forward compatible implementations.
type UnimplementedNodeSimulatorServer struct {
}

func (UnimplementedNodeSimulatorServer) Sent(context.Context, *Message) (*Message, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Sent not implemented")
}
func (UnimplementedNodeSimulatorServer) Received(context.Context, *Message) (*Message, error) {
	return nil, status.Errorf(codes.Unimplemented, "method Received not implemented")
}
func (UnimplementedNodeSimulatorServer) mustEmbedUnimplementedNodeSimulatorServer() {}

// UnsafeNodeSimulatorServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to NodeSimulatorServer will
// result in compilation errors.
type UnsafeNodeSimulatorServer interface {
	mustEmbedUnimplementedNodeSimulatorServer()
}

func RegisterNodeSimulatorServer(s grpc.ServiceRegistrar, srv NodeSimulatorServer) {
	s.RegisterService(&NodeSimulator_ServiceDesc, srv)
}

func _NodeSimulator_Sent_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(Message)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(NodeSimulatorServer).Sent(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/gopaxos.NodeSimulator/Sent",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(NodeSimulatorServer).Sent(ctx, req.(*Message))
	}
	return interceptor(ctx, in, info, handler)
}

func _NodeSimulator_Received_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(Message)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(NodeSimulatorServer).Received(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/gopaxos.NodeSimulator/Received",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(NodeSimulatorServer).Received(ctx, req.(*Message))
	}
	return interceptor(ctx, in, info, handler)
}

// NodeSimulator_ServiceDesc is the grpc.ServiceDesc for NodeSimulator service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var NodeSimulator_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "gopaxos.NodeSimulator",
	HandlerType: (*NodeSimulatorServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "Sent",
			Handler:    _NodeSimulator_Sent_Handler,
		},
		{
			MethodName: "Received",
			Handler:    _NodeSimulator_Received_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "gopaxos.proto",
